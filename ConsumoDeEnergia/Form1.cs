...
namespace ConsumoDeEnergia
{
    public partial class Form1 : Form
    {
        ...
        {
            DataGridViewCell cell = dgvLeitura.Rows[0].Cells[0];
            this.leituras.Add(new Leitura("Total", 0));

            for (int i = 0; i < 3; i++)
            {
                dgv.Rows[dgvLeitura.Rows.Count - 1].Cells[i].Style.BackColor = Color.Blue;
                dgv.Rows[dgvLeitura.Rows.Count - 1].Cells[i].Style.ForeColor = Color.Yellow;
                dgv.Rows[dgvLeitura.Rows.Count - 1].Cells[i].Style.Font = 
            ...
            lblConsumo.Text = "Total consumo sem desconto: " +
